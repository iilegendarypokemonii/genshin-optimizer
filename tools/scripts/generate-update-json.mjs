...
export function packageDesktopRelease(rootDir, tag) {
  ...
  writeFileSync(
    ...
    `Windows desktop release ${config.version} of this personal Genshin Optimizer modification.\n\n` +
      ...
      '**New: automatic inventory capture on Windows 10.** Game data now automatically selects a compatible capture method. You can also choose **Compatibility (Windows 10)** before starting capture. No extra driver installation is needed; approve the Windows administrator prompt and enter the game door. Compatibility capture uses IPv4; restart capture if you change networks or VPNs.\n\n' +
